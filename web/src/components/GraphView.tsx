...
export default function GraphView() {
  ...
  const fileList = useMemo(() => Object.values(nodes).filter((n) => n.type === "file"), [nodes]);
  ...
  const handleNodeDragEnd = useCallback((node: { id: string; x?: number; y?: number }) => {
    setGraphData((prev) => ({
      ...prev,
      nodes: prev.nodes.map((n) => (n.id === node.id ? { ...n, x: node.x, y: node.y } : n)),
    }));
  }, []);
